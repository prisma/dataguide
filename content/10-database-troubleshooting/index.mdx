...
The assumption for this article is that you're using a managed database on a cloud hosting provider. For example, Postgres deployed to DigitalOcean, MySQL deployed to AWS RDS, MongoDB on Atlas, etc. If, instead, you are fully managing your own database deployment on your own servers or on bare servers with cloud providers then you are likely already aware of the guidance outlined below (and likely more).
