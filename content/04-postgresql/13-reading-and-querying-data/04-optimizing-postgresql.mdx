...
**Note:** Using the `\gx` line termination sequence instead of the traditional semicolon (`;`) tells PostgreSQL to display to use expanded output mode for the current query.  This displays the columns and associated values for each record vertically instead of horizontally, which can improve readability in some cases.
...
The output will provide information about all locks within PostgreSQL which can help you diagnose contention between different processes control control of different objects.
...
Currently, the `log_min_duration_statement` setting is commented out with its current value set to `-1` to represent the default value.  The other settings are well-commented within the file and allow you to sample statements that are over the minimum instead of logging all of the statements.  The last setting show allows you to do sampling of statements within transactions as well.
...
Another option when trying to detect slow queries is to limit slow query logging to a specific database.  While you can  `log_min_duration_statement` can be set globally as we showed in the last section, it can also be configured at the database level.
