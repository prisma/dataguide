...
metaTitle: 'An introduction to PostgreSQL data types'
metaDescription: "PostgreSQL's data type system allows you to define your data structures to accept and store data in various formats.  In this guide, we'll introduce some of the most common PostgreSQL data types and show you how to work with them."
---

## Introduction

One of the primary features of relational databases in general is the ability to define [schemas](/intro/intro-to-schemas) or [table structures](/postgresql/create-and-delete-databases-and-tables#create-tables-within-databases) that exactly specify the format of the data they will contain.  This is done by prescribing the columns that these structures contain along with their *data type* and any constraints.
...
* **`integer`:** a normal, 4 byte integer that can range from -2147483648 to 2147483647.  This is the most common integer type to use as it represents a good trade off between storage and expressiveness for many practical applications.
* **`smallint`:** a 2 byte integer that can range from 32768 to 32767.  It is rare to use this except in places with tight storage constraints.
* **`bigint`:** an 8 byte integer that can be used to store values from -9223372036854775808 to 9223372036854775807.  Typically this type is reserved for scenarios where the `integer` type would have an insufficient range.
...
PostgreSQL's character and string types can be placed into two categories: *fixed length* and *variable length*.  The choice between these two affects how PostgreSQL allocates space for each value and how it validates input.
...
When a positive integer is provided in the declaration, the `char` column will store a fixed length string equal to the number of characters specified:
...
PostgreSQL uses the `boolean` type to express true and false values:

```
BOOLEAN
```

In keeping with [SQL standards](https://en.wikipedia.org/wiki/Three-valued_logic#SQL), the PostgreSQL boolean type can actually express three states:
...
The `time` data type (also called `time without time zone`) can store a specific time of the day without an associated timezone or date.
...
If you are using [Prisma Client](https://www.prisma.io/docs/reference/tools-and-interfaces/prisma-client) to work with your PostgreSQL databases, you can find a mapping between some of the common PostgreSQL and Prisma types in [Prisma's PostgreSQL data connectors docs](https://www.prisma.io/docs/reference/database-connectors/postgresql#type-mapping-between-postgresql-to-prisma-schema).
