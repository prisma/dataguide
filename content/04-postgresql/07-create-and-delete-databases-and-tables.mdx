...
Yes, PostgreSQL supports the use of `IF NOT EXISTS` when creating both databases and tables. The below demonstrates using the clause for table creation, and you can read more about its use [here](https://www.prisma.io/dataguide/postgresql/create-and-delete-databases-and-tables#create-tables-conditionally-with-the-if-not-exists-clause) 
...
To create a database from a dump, PostgreSQL provides the [utility program `pg_dump`](https://www.postgresql.org/docs/9.1/backup-dump.html).
...
To create a database in PostgreSQL, use the `CREATE DATABASE` command. The syntax is as follows:
