...
<details><summary>Does PostgreSQL support outer join?</summary>
...
<details><summary>What is Lateral Join in PostgreSQL?</summary> 

Yes, the `LATERAL` key word in [PostgreSQL](https://www.postgresql.org/docs/9.3/sql-select.html#SQL-FROM) can precede a sub-SELECT FROM item and allows the sub-SELECT to refer to columns of FROM items that appear before it in the FROM list. (Without LATERAL, each sub-SELECT is evaluated independently and so cannot cross-reference any other FROM item.)

</details>

<details><summary>Does PostgreSQL support cross join?</summary> 
...
An example output can be seen [here](https://www.prisma.io/dataguide/postgresql/reading-and-querying-data/joining-tables#cross-join)
