...
metaTitle: 'In Vivo: Information Ecosystems'
...
Still other "mineral" database designs cede even that minimal authority over information, providing only storage with the bare minimum of structure like the five-thousand-year-old clay tablets onto which the Sumerians stamped everything from ledgers to complaints. It does happen that a situation calls for little more than an ACID-compliant^1 spreadsheet, but looking more closely at the system is well warranted. It's also possible that an existing "mineral" database's implementation fails to account for or awkwardly approximates a more complex data model, or that part or all of the data model itself is ill-suited to relational representation.

## Databases, Plural

Every program or system -- not to exclude designers, users, or developers -- interacting with a database also has an ecological role, consuming and generating information, combining it, separating it, adding or eliminating nuance for good or ill. Many of these interacting systems have their own data models, of which part or all of the target system's data model now forms a component (sometimes through the proverbial glass darkly, as anyone who's ever cursed an unexpected `VARCHAR(20)` can attest).
...
| If you need to store and process...                                                                            | Look towards...^4.                                                                                    |
|----------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------|
...
^4 Kristof Kovacs maintains a [more detailed summary](https://kkovacs.eu/cassandra-vs-mongodb-vs-couchdb-vs-redis) of many NoSQL options, from the popular to the obscure.
...
The most common form of replication involves writing data only to a single primary^5 server. The primary forwards changes to one or more replica servers, which either stand by to replace it should it go down (failover) or shoulder the work of responding to read queries. In the latter case network latency can mean that changes might not be visible immediately, but since they only flow one way, consistency is still guaranteed. This division of reads and writes lends itself naturally to representing consolidated entities in views where appropriate, and in software systems especially to [command query responsibility segregation](https://www.martinfowler.com/bliki/CQRS.html), a design pattern which can help control complexities appertaining only to retrieval or to storage.
...
Distributed databases are often described as favoring data *consistency* or *availability* in the inevitable event of a network partition preventing some set of nodes from communicating writes to other nodes, after Eric Brewer's original [conjecture](https://people.eecs.berkeley.edu/~brewer/cs262b-2004/PODC-keynote.pdf) around the turn of the millennium. His "CAP Theorem" (consistency, availability, partition tolerance: pick two) still resonates today, but, like the animal/vegetable/mineral trichotomy, is an elegant oversimplification -- no more and no less.
...
[ illustration - ETL flow from external systems through cron to a database ]
...
^1 [ACID](https://en.wikipedia.org/wiki/ACID) denotes a common goal of relational databases to guarantee *atomicity* (any transaction succeeds in whole or fails in whole), *consistency* (the database cannot be put into an invalid state), *isolation* (independent transactions do not affect each other), and *durability* (committed transactions cannot be lost to crash or sudden shutdown).
^2 In fairness, MySQL does allow queries and foreign key constraints across databases, which takes most of the sting out.
^3 Without an appropriate `ON DELETE` directive in the foreign key definition, of course.
^4 These are examples, not endorsements!
^5 Sometimes "leader", etc.; "master" is still seen but is widely and not-inaccurately considered gauche at best, the original usage here being in the sense of "master" and "slave" servers.