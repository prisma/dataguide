...
metaTitle: '5 Ways to host PostgreSQL databases'
metaDescription: "There are a variety of ways to get PostgreSQL servers for your projects.  You can install and run your databases yourself or use a managed database service.  This guide will discuss the advantages and disadvantages of various PostgreSQL hosting options."
metaImage: /content/postgresql/ways-to-host-postgresql-databases/header.png
toc: false
---
...
<details><summary>Some readers will already know about this to one degree or another. It's worth including for those who won't, especially if their experience so far has been only of "dumb" databases backing O/RM-served applications.</summary>


One named block of SQL managed and executed in the database seems like it shouldn't be too different from another, but most RDBMSs have distinguished functions and stored procedures for decades. As with other aspects of implementing the SQL standard, the exact details vary a bit from RDBMS to RDBMS. In general:
...
<footnote> <note> If you are more familiar with a C-family anguage than you are with PostgreSQL, the <inlineCode>||</inlineCode> might throw you. That's concatenation here.</note> <text> <i>^1</i> </text> </footnote>
...
```
<footnote> <note> In keeping with this chapter's theme of vendor fragmentation, PostgreSQL requires the trigger invoke a separate function with a return type of <inlineCode>TRIGGER</inlineCode>; MySQL instead accepts a <inlineCode>BEGIN ... END</inlineCode> block as a trigger body; Oracle and SQL Server can do either.</note> <text> <i>^5</i> </text> </footnote> 

Once you start executing DML<footnote> <note> Data Manipulation Language: the other half of SQL, <inlineCode>SELECT</inlineCode>, <inlineCode>INSERT</inlineCode>, <inlineCode>UPDATE</inlineCode>, and <inlineCode>DELETE</inlineCode>.</note> <text> <i>^6</i> </text> </footnote> and using flow control, it's a hop skip and jump to kicking off much more complicated sequences of steps and expanding the scope to include still other tables. Perhaps the insertion of a record into `lots` itself may be the final operation of a trigger initiated by an insert into `orders`, with no human user or application backend empowered to write to `lots` directly. Or as `items` are added to a lot, a trigger there might handle updating `current_quantity`, and start some other process when it reaches the `target_quantity`.
...
[ illustration - network of cascade effects as a change fires a trigger, which makes more changes, which fire more triggers in turn, quickly becoming an incomprehensible tangle ]
...
^1: If you are more familiar with a C-family language than you are with PostgreSQL, the `||` might throw you. That's concatenation here.
^2: If an insert fails, for example by supplying a negative `target_quantity`, `lot_counter` will still have incremented. This leaves a confusing but otherwise harmless gap in `lot_number`.
^3: "What about `SELECT ... INSERT`?", I hear you cry. Yes, as long as you `UNION` a dummy value in for the first lot of the year when `count()` has nothing to count, that will indeed work! Pretending it doesn't is much easier than coming up with another example.
^4: Data Definition Language: the structural half of SQL with the `DROP TABLE`s and the `CREATE OR REPLACE FUNCTION`s and so forth.
^5: In keeping with this chapter's theme of vendor fragmentation, PostgreSQL requires the trigger invoke a separate function with a return type of `TRIGGER`; MySQL instead accepts a `BEGIN ... END` block as a trigger body; Oracle and SQL Server can do either.
^6: Data Manipulation Language: the other half of SQL, `SELECT`, `INSERT`, `UPDATE`, and `DELETE`.

<AuthorInfo name="Dian Fay" image="https://i.imgur.com/AtYiUlw.jpg">

 Dian didn’t exactly plan to drop out of college to specialize in SQL and backend development, but that’s how it happened. Fifteen years later, she’s designed databases supporting everything from industrial logistics and traceability systems to million-plus user social media games. She is the current maintainer of [MassiveJS](https://massivejs.org), an open source data mapper for Node.js focused on using PostgreSQL to the fullest.
</AuthorInfo>
