...
One of the most commonly discussed challenges when working with serverless is called the [*cold start* problem](https://www.prisma.io/dataguide/serverless/serverless-glossary#cold-start).  While the goal with serverless is allow functions to immediately be executed on demand, there are some scenarios that may result in predictable delays.
...
One challenge that is difficult to get away from with serverless is [provider lock-in](https://www.prisma.io/dataguide/serverless/serverless-glossary#vendor-lock-in).  When you architect your application to rely on external functions running on a specific provider's platform, it can be difficult to migrate to a different platform at a later time.
