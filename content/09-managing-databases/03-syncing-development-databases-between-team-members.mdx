...
Once developers are set up with a database to work against, whether it be local or remote, they will need to continuously check for updates to the database schema and possibly the data in the database itself. This can be difficult if team sizes are large and changes are made frequently.
...
The following serves as a set of possbilities for keeping data and schema changes in-sync across your teams.
...
This becomes a problem when two or more team members are working on the same part of the database schema or part of the application that consumes a given part of the schema. Such a scenario is likely to produce mege conflicts and cause frustration.
