...
On the other side of things, document databases define the structure of each individual document separately. The structure is a characteristic that the document itself defines rather than an _external_ structure the record must conform to.
...
If you are using [Prisma to manage your MongoDB database](https://www.prisma.io/docs/orm/overview/databases/mongodb), you need to set a connection URI within a 'datasource' block in your [Prisma schema file](https://www.prisma.io/docs/orm/prisma-schema/overview). You must provide a [connection URI for the 'url' field](https://www.prisma.io/docs/orm/overview/databases/mongodb#example) so that Prisma can connect to your database.
