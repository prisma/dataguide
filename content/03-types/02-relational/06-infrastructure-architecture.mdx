...
![CQRS with read-only replicas diagram](../../dataguide-images/types/relational/infrastructure-architecture/cqrs.png)

_Command query responsibility segregation_ (CQRS) is a term used to describe adding logic to separate out queries that mutate data ([write queries](/intro/database-glossary#write-operation)) from those that do not ([read queries](/intro/database-glossary#read-operation)). This allows you to route these different categories of requests to different hosts to help distribute the load.
...
![Adding a read cache diagram](../../dataguide-images/types/relational/infrastructure-architecture/read-cache.png)

To decrease response times, a _read query caching_ layer can be introduced. Adding a [cache](/intro/database-glossary#cache) between your database clients and the databases themselves can reduce query time for common requests significantly. The application can request read results from the cache and receive them almost immediately if available. For cases where the results are not found in the cache, they are fetched from the database itself and added to the cache for next time.
...
![Data sharding diagram](../../dataguide-images/types/relational/infrastructure-architecture/data-sharding.png)
...
![Functional data management diagram](../../dataguide-images/types/relational/infrastructure-architecture/functional-separation.png)
...
For many organizations, this strategy has important advantages that go beyond the properties of the actual systems. Decentralizing data management can allow smaller teams to own their own data without coordinating changes with other parties. It aligns well with the focused separation of concerns promoted by [microservice-oriented application architectures](/intro/database-glossary#microservice-architecture).
...
![Serverless database architecture diagram](../../dataguide-images/types/relational/infrastructure-architecture/serverless-databases.png)

_Serverless databases_ are a category of services that decouple data storage from data processing to easily scale resources in response to changes in demand.
...
With a CQRS design pattern, you can separate your concerns and database responsibilities that a single database would undertake in a [monolithic architecture](/intro/database-glossary#monolithic-architecture).
