...
metaImage: '/dataguide-images/types/relational/expand-and-contract-pattern/header-image.png'
---

## Introduction

![Expand and contract header image](/dataguide-images/types/relational/expand-and-contract-pattern/header-image.png)

[Database schema](/intro/database-glossary#schema) designs inevitably evolve over time as the software that interacts with the database change. It is important to be able to transition your schemas to incorporate new requirements as they arise in a safe and reliable manner.

While [database migration](/intro/database-glossary#migration) software can often be used to update the actual data structures, the data itself must also be converted to new formats, migrated to different tables or columns, and updated to conform to new expectations. These processes, while not always complicated, are especially sensitive when performed on live systems that are actively being used by clients.

Fortunately, a multi-step process known as the _expand and contract pattern_ can be applied to safely migrate clients to use new data structures incrementally without taking the system offline. In this guide, we will explore what the expand and contract pattern is, how it enables seamless transitions between new and old structures, and how to apply it to various database transformations.

## What is the expand and contract pattern?

The expand and contract pattern is a process that [database administrators](/intro/database-glossary#database-administrator) and software developers can use to transition data from an old data structure to a new data structure without affecting uptime. It works by applying changes through a series of discrete steps designed to introduce the new structure in the background, prepare the data for live usage, and then switch over to the new structure seamlessly.
...
![build and deploy new schema](/dataguide-images/types/relational/expand-and-contract-pattern/step-1.png)
...
![expand the interface](/dataguide-images/types/relational/expand-and-contract-pattern/step-2.png)

After deploying the new database schema alongside the original schema, the next step is to expand the interface of the clients to recognize the new columns and tables. The client code will still read all of its values from the original data structure. However, when performing [write operations](/intro/database-glossary#write-operation), the client will now add or modify data in both the original structure and in the new structure.
...
![migrate existing data](/dataguide-images/types/relational/expand-and-contract-pattern/step-3.png)
...
To perform migrations with [Prisma](https://github.com/prisma/prisma), you can use the [Prisma Migrate](https://www.prisma.io/docs/concepts/components/prisma-migrate). [Developing with Prisma Migrate](https://www.prisma.io/docs/guides/database/developing-with-prisma-migrate) generates migration files based on the declarative [Prisma schema](https://www.prisma.io/docs/concepts/components/prisma-schema) and applies them to your database.

</PrismaOutlinks>

### Step 4: Test the new interface

![test the new interface](/dataguide-images/types/relational/expand-and-contract-pattern/step-4.png)
...
![read from new schema](/dataguide-images/types/relational/expand-and-contract-pattern/step-5.png)
...
![stop writing to original](/dataguide-images/types/relational/expand-and-contract-pattern/step-6.png)
...
![remove original schema](/dataguide-images/types/relational/expand-and-contract-pattern/step-7.png)
...
Next, the client application is updated to write any new playground-related data to both the `equipment` table and the `playground` table by configuring the application to write to all tables returned by the feature flag `DATABASE:equipment_location:write` defined as a list on their [Redis](/intro/database-glossary#redis) server. While they are adding this change, they also implement another feature flag that checks the value of `DATABASE:equipment_location:read` to determine which data source to read from.
...
Both tables are now up-to-date. It is important to note, however, that the new `playground` table _does_ contain information not stored in the `equipment` table (the `sq_ft` column). If you were to roll back changes at this point, you would be able to return to your previous table state, but you would lose the data that's been captured for each playground's size.
...
[Prisma Client](https://www.prisma.io/docs/concepts/components/prisma-client), check out the matching section in [Prisma's documentation on advanced migration scenarios](https://www.prisma.io/docs/guides/database/developing-with-prisma-migrate/customizing-migrations).
