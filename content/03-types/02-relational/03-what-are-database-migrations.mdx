...
metaTitle: 'Database Migrations: What are the Types of DB Migrations?'
...
metaImage: '../../dataguide-images/types/relational/what-are-database-migrations/header-image.png'
---

## Introduction

![migrations header image](../../dataguide-images/types/relational/what-are-database-migrations/header-image.png)

Database [schemas](/intro/database-glossary#schema) define the structure and interrelations of data managed by relational databases. While it is important to develop a well-thought out schema at the beginning of your projects, evolving requirements make changes to your initial schema difficult or impossible to avoid. And since the schema manages the shape and boundaries of your data, changes must be carefully applied to match the expectations of the applications that use it and avoid losing data currently held by the database system.
...
Database migrations, also known as [schema migrations](/intro/database-glossary#migration), database schema migrations, or simply migrations, are controlled sets of changes developed to modify the structure of the objects within a [relational database](/intro/database-glossary#relational-database). Migrations help transition database schemas from their current state to a new desired state, whether that involves adding tables and [columns](/intro/database-glossary#column), removing elements, splitting fields, or changing types and [constraints](/intro/database-glossary#constraint).
...
To perform migrations with [Prisma](https://github.com/prisma/prisma), you can use the [Prisma Migrate](https://www.prisma.io/docs/orm/prisma-migrate). Prisma Migrate generates migration files based on the declarative [Prisma schema](https://www.prisma.io/docs/orm/prisma-schema/overview) and applies them to your database.
