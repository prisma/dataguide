...
The declaration syntax for a `DECIMAL` column is `DECIMAL(M, D). The ranges of values for the argumments are as follows:
...
The [storage requirements](https://dev.mysql.com/doc/refman/8.0/en/storage-requirements.html) for string types in MySQL can be represented in the following table with *M* representing the declared column length in characters and *L* representing the actual length in bytes of a given string value.
