...
* `BLOCK IO`: Display number of blocks of input and output the query generated
...
You can do this with the [`LIMIT` clause](https://dev.mysql.com/doc/refman/8.0/en/limit-optimization.html).  For example, to return only the five results, you can use a format like this:

```sql
SELECT * FROM <table> LIMIT 5
```

In many cases (depending on what other clauses are included in the query), limiting the number of rows returned can help MySQL short circuit the querying process, stopping sooner than it would normally need to.  This helps save time early on instead of executing over the entire dataset early on and then truncating the results later.
...
If you are using Prisma with your MySQL database, you can about ways to optimize your queries in the [query optimization section of the docs](https://www.prisma.io/docs/guides/performance-and-optimization/query-optimization-performance).  This will help you understand how various query constructions can impact your database performance when using Prisma.
