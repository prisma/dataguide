...
[Non-repeatable reads](/intro/database-glossary#nonrepeatable-read) occur when a commit outside of the transaction alters the data seen within the transaction.  You can recognize this type of problem if, within a transaction, the same data is read twice but different values are retrieved in each instance.
