...
If you are connecting to a local MongoDB server that _has_ been configured with access control, you will need to provide additional information to connect. You will need to provide at least a username and password to connect using the associated `--username` and `--password` options:
