...
```
Deciding on a document database versus a relational database comes down to the type of data that the database is storing and how it shoud be accessed. MongoDB's documents promote flexibility over rigidity and case ease the way data is accessed from an application.
