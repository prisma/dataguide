...
[MongoDB Atlas] supports [transport layer security (TSL)] and secure socket layer (SSL) for communications between application client and server and within intra-cluster communications by setting certificates for the servers. A certificate is an electronic document used to prove the validity of ownership of a public key. Certificates ensure data is encrypted for transport over a trusted network connection preventing network snooping activities such as packet sniffing or IP/DNS spoofing. 

With the inclusion of a public key, we see that MongoDB utilizes asymmetric encryption to secure data in a state of motion. This is because of an inherent vulnerability in symmetric encryption for data in transit. The secret symmetric key must remain secure during the transmission. To secure this original key, you must encrypt it and create another key to decrypt. This cycle can continue infinitely, where every key in transmission is then reliant on another made key to secure it. Asymmetric encryption theoretically solves this insecure transmission problem with its public keys. 
