...
MongoDB provides two [APIs](https://docs.mongodb.com/manual/core/transactions/#transactions-api) to use transactions. The first is the core API which has similar syntax to relational databases. The second, the callback API, is the recommended approach to using transactions in MongoDB.
...
```
Now that we have our database, we can show the steps to work with a transaction directly in the shell and demonstrate how a transaction being conducted in one session may not be detectable to an outside source.
...
```
This newly created variable will work the as `db.authors` would when working outside of the transaction session. You can verify this by opening a second shell window, connecting to your cluster, and running `db.authors.find()`. It will return the same documents.
...
```
If we run an `authors.find()` now in the session we'll return the following:
...
Since this transaction session has still not been committed, we will not see it return the same result in a shell existing outside of the session. To confirm, we run `db.authors.find()` in another shell instance and we return the original documents.
...
```
This difference demonstrates how the transaction is still currently in a state of ambiguity. It can still either succeed and be committed to the database or it could fail. Either way the database will come back to a consistent state of either including the new record or remaining the same as before the transaction session started.

To commit the transaction and bring the database to the new consistent state with the new author, we run the following in the session context:

```
session.commitTransaction()
```  
Now, the new record will exist in both instances, the shell where the transaction session was taking place as well as any shell outside of the transaction.
  

### Aborting a transaction

Now that we have gone through starting a session and working all the way to committing it, we can explore the alternative result for a transaction.
...
```
This method cancels the transaction session and discards any potential changes. The database returns to its original state. Instead of resulting in a new consistent state, the database rolls back the changes and stays the same as when the transaction session first started.  

## Conclusion

In this guide, we discussed what transactions are and what use cases transactions suit best in MongoDB. We also walked through the process of a transaction session conceptually in the MongoDB shell so you can understand how and external application will operate.

Transactions are a fundamental need for relational databases, and are also needed for specific use cases in document model databases such as MongoDB. When it comes to the document model, the general recommendation is that data that is accessed together should be stored together. However, sometimes this cannot be the case, so it is important to know about transactions.
<PrismaOutlinks>
