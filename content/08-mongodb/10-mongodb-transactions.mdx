...
```
If we run `authors.find()` now in the session we'll return the following:
...
```  
Now, the new record will exist in both instances, the MongoDB shell where the transaction session was taking place as well as any MongoDB shell outside of the transaction.
