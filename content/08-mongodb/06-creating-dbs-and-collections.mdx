...
MongoDB uses [document-oriented structures](/intro/database-glossarydocument-database) to store, manage, and process data.  Individual [documents](/intro/database-glossary#document) are organized into [collections](/intro/database-glossary#collection), which in turn, are stored in databases.  Because the schema of each document is not defined by a static schema, document based systems offer more flexibility than relational systems that are composed of tables and records.
