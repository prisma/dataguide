...
metaTitle: 'Connecting to PostgreSQL databases'
...
In a companion guide, you can find out how to [configure PostgreSQL's authentication to meet your project's needs](/postgresql/configuring-user-authentication).  Consider reading both guides for a more complete picture of how authentication works in PostgreSQL.
...
However, it's unlikely that your normal operating system username already has an associated PostgreSQL username.  So usually, you'll want need to 
...
This same information can also be encoded into a [PostgreSQL *connection string*](https://www.postgresql.org/docs/current/libpq-connect.html#id-1.7.3.8.3.6).  A connection string provides the same information in a single URI string that uses certain characters as delimiters between the different fields.
...
```bash
psql postgresql://myappplicationuser:mypass@myhost:1234/applicationdb
```
...
If you want to modify the rules that dictate how users can authenticate to your PostgreSQL instances, you can do so by modifying your server's configuration.  You can find out [how to modify PostgreSQL's authentication configuration in this article](/postgresql/configuring-user-authentication).
