---
title: 'Setting up a local SQLite database'
metaTitle: "Setup and Connect Local Database | SQLite | Prisma's Data Guide"
metaDescription: 'Learn how to set up a SQLite database on Windows, macOS, and Linux by following the step-by-step instructions in this practical guide.'
...
If you are using SQLite with Prisma, you can follow our [quick start guide](https://www.prisma.io/docs/getting-started/quickstart), which comes with a ready-to-go database.

</PrismaOutlinks>

## Setting up SQLite on Windows

The [SQLite](/intro/database-glossary#sqlite) project provides a zipped bundle of tools that include the `sqlite.exe` file that you need to create and interact with SQLite databases from the command line.
...
![SQLite download page](/dataguide-images/sqlite/setting-up-a-local-database/windows/download_page.png)
...
![SQLite zip file](/dataguide-images/sqlite/setting-up-a-local-database/windows/downloads.png)

Right-click the file and select **Extract All...** to bring up a new extract window:

![SQLite extract window](/dataguide-images/sqlite/setting-up-a-local-database/windows/extract_menu.png)
...
![SQLite select desktop](/dataguide-images/sqlite/setting-up-a-local-database/windows/extract_folder.png)
...
![SQLite tools in folder](/dataguide-images/sqlite/setting-up-a-local-database/windows/sqlite_directory.png)

If you click the `sqlite3.exe` application, a new window will appear with an interactive SQLite session:

![new SQLite session](/dataguide-images/sqlite/setting-up-a-local-database/windows/sqlite_no_db.png)

Notice that SQLite is currently using an [in-memory database](/intro/database-glossary#in-memory-database). This means that it is not [reading](/intro/database-glossary#read-operation) from or [writing](/intro/database-glossary#write-operation) to a file currently.

To make SQLite write to a new database file, type `.open --new` followed by the database file name you wish to use:

![SQLite open new database](/dataguide-images/sqlite/setting-up-a-local-database/windows/sqlite_open_new_db.png)
...
![SQLite open database](/dataguide-images/sqlite/setting-up-a-local-database/windows/sqlite_open_db.png)
...
![SQLite open database](/dataguide-images/sqlite/setting-up-a-local-database/windows/sqlite_from_cmd.png)
...
![SQLite download page](/dataguide-images/sqlite/setting-up-a-local-database/macos/download_page.png)

Save the file to a convenient location.

Open the Finder file manager and navigate to the location of the zipped archive:

![SQLite zip file](/dataguide-images/sqlite/setting-up-a-local-database/macos/zip_file.png)
...
![SQLite SQLite tools](/dataguide-images/sqlite/setting-up-a-local-database/macos/sqlite_tools.png)
...
If you run the command without any arguments, SQLite will use an in-memory database:
...
![SQLite download page](/dataguide-images/sqlite/setting-up-a-local-database/linux/download_page.png)
